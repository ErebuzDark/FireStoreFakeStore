...
const Home = () => {
  ...
  const [isViewProDuctDetailsVisible, setIsViewProDuctDetailsVisible] =
    useState(false);
  ...
  useEffect(() => {
    if (
      absoluteOpenModal === "true" &&
      absoluteProductID &&
      products.length > 0
    ) {
      const foundProduct = products.find(
        (product) => product.id === Number(absoluteProductID)
      );
      if (foundProduct) {
    ...
    <>
      <div className="flex gap-2 w-full p-3 md:p-10">
        ...
        <Dropdown label="Sort By" color="light" className="w-fit text-nowrap">
          <DropdownItem onClick={() => handleSortAndKey("price", "asc")}>
            Price: Low to High
          </DropdownItem>
          <DropdownItem onClick={() => handleSortAndKey("price", "desc")}>
            Price: High to Low
          </DropdownItem>
          <DropdownItem onClick={() => handleSortAndKey("rating", "asc")}>
            Rating: Low to High
          </DropdownItem>
          <DropdownItem onClick={() => handleSortAndKey("rating", "desc")}>
            Rating: High to Low
          </DropdownItem>
          <DropdownItem onClick={() => handleSortAndKey("title", "asc")}>
            Title: A to Z
          </DropdownItem>
          <DropdownItem onClick={() => handleSortAndKey("title", "desc")}>
            Title: Z to A
          </DropdownItem>
        </Dropdown>
      </div>
      ...
      <Modal
        size="3xl"
        show={isViewProDuctDetailsVisible}
        dismissible
        ...
        <ModalBody>
          <div className="space-y-6">
            <img
              ...
              alt=""
              className="product-img w-1/3 mx-auto my-8"
            />
            <div>
              <h2 className="text-2xl font-medium">{selectedProduct?.title}</h2>
              <small className="text-base text-slate-500">
              ...
              <p className="my-3 text-xs">
                <span
                  className={`${
                    ...
                    (selectedProduct?.rating?.rate <= 3.9 &&
                      "text-yellow-400") ||
      ...
      {/* <Modal show={null} size="lg" dismissible>
        ...
        <ModalBody>
          <form className="flex flex-col gap-2">
            <TextInput onChange={handleChange} className="w-full" name="firstName" type="text" placeholder="First Name" />
            <TextInput onChange={handleChange} className="w-full" name="lastName" type="text" placeholder="Last Name" />
            <TextInput onChange={handleChange} className="w-full" name="age" type="number" placeholder="Age" />
            <TextInput onChange={handleChange} className="w-full" name="email" type="email" placeholder="Email" />
          </form>
        </ModalBody>
        <ModalFooter>
